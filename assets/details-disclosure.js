...
class SideDrawer extends HTMLElement {
  ...
  open(triggeredBy) {
    ...
    if (this.classList.contains('drawer--menu-multi-tab')) {
      const $header = document.querySelector('header');

      if ($header) {
        const verticalPosition = $header.getBoundingClientRect().bottom;
        this.style.setProperty('--vertical-position', verticalPosition + 'px');
      }
    }
...
class MegaMenuMansory extends HTMLElement {
  constructor() {
    super();
  }
  ...
  renderContent() {
    ...
    new Masonry(mansory, {
      ...
      columnWidth: '.mansory-item',
      percentPosition: true,
      gutter: 16
    });
  }
}
if (!customElements.get('mega-menu-mansory')) customElements.define('mega-menu-mansory', MegaMenuMansory);
